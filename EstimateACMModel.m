...
shortTermInterestRate = yields(:, 1) * sixMonths2FirstMonth;
numFactors = 4;
maturitiesMonths = 12 * maturities;

decomposition = estimateACM(yields, shortTermInterestRate, maturitiesMonths, numFactors );


figure
hold on
plot( T.Time, yields(:, end), "LineWidth", 2 )
plot( T.Time, decomposition.riskPremium(:, end), "LineWidth", 2 )
plot( T.Time, decomposition.expected(:, end), "LineWidth", 2 )
legend( "Actual", "Term Premium", "Expected" )
...
% % 
% % where 
% %   * y is the continuously compounded yield ("log yield")
...
% sixMonths2FirstMonth = 1 / 6;
% shortTermInterestRate = yieldMatrix(:, 1) * ...
%     sixMonths2FirstMonth * percent2proportion; 
% 
% % Compute the excess returns using equation (6) from Adrian, Crump and 
% % Moenich, 2013. Here, the rows represent continuous time
...
% q0 = mu - lambda0;
% qx = Phi - lambda1;
% A = NaN( numMaturities, 1 );
% B = NaN( numMaturities, numComponents );
% AP = NaN( numMaturities, 1 );
% BP = NaN( numMaturities, numComponents );
% AQ = NaN( numMaturities, 1 );
% BQ = NaN( numMaturities, numComponents );
% 
% % TODO: Refactor to avoid transposing B.
...
%             normTrace) - delta0;
%         B(n, :) = B(n-1, :) * qx - delta1.';
%         AP(n, 1) = AP(n-1, 1) + BP(n-1, :) * mu - delta0;
%         BP(n, :) = -delta1.' + BP(n-1, :) * Phi;
%         AQ(n, 1) = AQ(n-1, 1) - delta0 + BQ(n-1, :) * q0;
%         BQ(n, :) = BQ(n-1, :) * qx - delta1.';        
%     end % if
% end % for
% 
% % Convert from prices to yields.
% proportion2percent = 100;
% A = -proportion2percent * A ./ maturities;
% B = -proportion2percent * B ./ maturities;
% AP = -proportion2percent * AP ./ maturities;
% BP = -proportion2percent * BP ./ maturities;
% AQ = -proportion2percent * AQ ./ maturities;
% BQ = -proportion2percent * BQ ./ maturities;
% 
% % Compute the fitted yields, 
% riskNeutralYield = AP + BP * selectedScores.';
% termPremium = AQ - AP + (BQ-BP) * selectedScores.';
% convexity = A-AQ;